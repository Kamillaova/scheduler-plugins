...
const (
	...
	ccxLargestCacheAttr  = "dra.cpu/largestUncoreCacheCPUs"
...
func makeCCXCacheDevice(name string, numaID int64, cacheID int64, capacity int, partition, repairRounds, frontierInput string) resourceapi.Device {
	return makeCCXMirroredCacheDevice(name, numaID, cacheID, capacity, capacity, partition, repairRounds, frontierInput)
}

// makeCCXMirroredCacheDevice publishes a capacity that differs from the cache's
// real size, which is what the driver's capacity mirror does once a claim has
// left a cache or squats on one. Slices built with equal values cannot tell a
// reader of the capacity apart from a reader of the topology.
func makeCCXMirroredCacheDevice(name string, numaID int64, cacheID int64, publishedCapacity, trueCacheCPUs int, partition, repairRounds, frontierInput string) resourceapi.Device {
	...
	dev := resourceapi.Device{
		...
		Attributes: map[resourceapi.QualifiedName]resourceapi.DeviceAttribute{
			...
			ccxLargestCacheAttr: {IntValue: &largest},
		},
...
func TestCCXAlign_CodebookGoldenVectors(t *testing.T) {
	...
	cases := []goldenVectorCase{
		...
		{
			name: "MirroredCapacityPacking: the cache size comes from topology, not from the mirrored capacity",
			mode: ccxalign.MostAllocated,
			devices: []resourceapi.Device{
				// cache-0 is squatted, so the driver floors what it publishes
				// far below the 16 CPUs the cache holds.
				makeCCXMirroredCacheDevice("cache-0", 0, 0, 4, 16, "default", "", ""),
				makeCCXMirroredCacheDevice("cache-1", 0, 1, 16, 16, "default", "", ""),
			},
			occupiedDevs:  []string{"cache-0"},
			claim:         makeCCXClaim("default", "c9", 1, 16, v1alpha1.AlignmentBestEffort, false, "default", false),
			minBand:       80,
			maxBand:       88,
			expectedScore: 81,
			expectedTier:  ccxalign.TierLabelWarmAligned,
