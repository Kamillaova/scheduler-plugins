...
// debitUnallocatedClaim takes the claim's demand out of the overlay without
// judging where it lands. Filter gates only claims asking for repair, but every
// claim of the pod consumes capacity that a later one is measured against.
func debitUnallocatedClaim(om *overlayMap, req *resourceapi.DeviceRequest, policy nodePolicy) {
	if om == nil || req == nil {
		return
	}
	count, raw := requestDemand(req)
	rem := count * policy.round(raw)
...
// getClaimPartition reads the partition from the claim's request for this
// driver's devices. Another driver's request may carry a toleration for a
// partition key of its own, which says nothing about where these CPUs go.
func getClaimPartition(claim *resourceapi.ResourceClaim, isDriverClass func(string) bool) string {
	req := driverRequest(claim, isDriverClass)
	if req == nil {
		return defaultPartition
	}
	if req.Exactly != nil {
		if part := partitionFrom(req.Exactly.Tolerations, req.Exactly.Selectors); part != "" {
			return part
		}
	}
	for _, sub := range req.FirstAvailable {
		if part := partitionFrom(sub.Tolerations, sub.Selectors); part != "" {
			return part
		}
	}
	return defaultPartition
...
func partitionFrom(tolerations []resourceapi.DeviceToleration, selectors []resourceapi.DeviceSelector) string {
	...
	for _, sel := range selectors {
		if sel.CEL == nil || !strings.Contains(sel.CEL.Expression, "partition") {
			continue
		}
...
// driverRequest returns the claim's request for this driver's devices. A claim
// may ask for another driver's device as well, and that request carries its own
// count and capacity; taking the first request would then size the claim from a
// demand that is not for CPUs at all.
func driverRequest(claim *resourceapi.ResourceClaim, isDriverClass func(string) bool) *resourceapi.DeviceRequest {
	for i := range claim.Spec.Devices.Requests {
		req := &claim.Spec.Devices.Requests[i]
		if req.Exactly != nil && isDriverClass(req.Exactly.DeviceClassName) {
			return req
		}
		for _, sub := range req.FirstAvailable {
			if isDriverClass(sub.DeviceClassName) {
				return req
			}
		}
	}
...
// requestDemand reports the number of results the request asks for and the
// per-result capacity it requests, reading the aligned alternative when the
// request offers alternatives.
func requestDemand(req *resourceapi.DeviceRequest) (count int, rawCPUs int) {
	count = 1
	if req == nil {
		return count, 0
	}
	...
	case req.Exactly != nil:
		...
		if req.Exactly.Capacity != nil {
			if qty, ok := req.Exactly.Capacity.Requests[capacityName]; ok {
				rawCPUs = int(qty.Value())
			}
		}
	case len(req.FirstAvailable) > 0:
		count = int(req.FirstAvailable[0].Count)
		if req.FirstAvailable[0].Capacity != nil {
			if qty, ok := req.FirstAvailable[0].Capacity.Requests[capacityName]; ok {
				rawCPUs = int(qty.Value())
			}
		}
	}
...
// cacheSize is the number of CPUs one whole uncore cache holds, which is what
// the claim's shape is measured against. It comes from the topology attribute
// rather than from a published capacity: capacity carries the driver's mirror
// correction, so it drifts as claims move and is floored on a squatted cache,
// and it describes one device where caches on a node may differ in size.
func (om *overlayMap) cacheSize() int {
	largest := 0
	for _, d := range om.devices {
		if d.largestCache > largest {
			largest = d.largestCache
		}
	}
	if largest > 0 {
		return largest
...
func evaluateSingleClaim(
	...
	isDriverClass func(string) bool,
) claimEvaluation {
	if len(om.devices) == 0 {
	...
	cpuReq := driverRequest(claim, isDriverClass)
	...
	if shape == shapeFlexible && cpuReq != nil && len(cpuReq.FirstAvailable) > 1 {
...
func scorePodOnNode(
	...
	claimPartition := getClaimPartition(claims[0], isDriverClass)
	...
	for _, claim := range claims {
		eval := evaluateSingleClaim(claim, om, policy, isCold, mode, isDriverClass)
...
func ScoreTestClaim(
	...
	return scorePodOnNode(claims, rawDevices, allocatedState, policy, mode, func(className string) bool {
