...
const (
	...
	largestCacheAttr  = "dra.cpu/largestUncoreCacheCPUs"

	defaultPartition = "default"
	defaultCacheSize = 16

	// gatherAttempts bounds the retry of a gather the claim tracker refuses
	// because a concurrent modification changed its revision.
	gatherAttempts = 3
...
type alignState struct {
	...
	allocated     *structured.AllocatedState
	allClaims     []*resourceapi.ResourceClaim
	mu            sync.Mutex
	predictedNUMA map[string]int
	inFlightNUMA  map[string]map[int]bool
}

func newAlignState(claims []*resourceapi.ResourceClaim, allocated *structured.AllocatedState, allClaims []*resourceapi.ResourceClaim) *alignState {
	return &alignState{
		claims:        claims,
		allocated:     allocated,
		allClaims:     allClaims,
