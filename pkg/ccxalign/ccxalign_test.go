...
func TestAbsoluteCodebookScoringPackingAndSpreading(t *testing.T) {
	...
	stateWarm := framework.NewCycleState()
	pluginPreFilter(pluginPreFilterArgs{t, packingWarmPlugin, stateWarm, podAligned})
	scoreWarm, _ := packingWarmPlugin.Score(context.Background(), stateWarm, podAligned, nodeInfoOf(nodeWarm))
...
func TestEnqueueExtensions_ResourceSliceRequeue(t *testing.T) {
	...
	hasClaimEvent := false
	for _, e := range events {
		if e.Event.Resource == fwk.ResourceClaim && (e.Event.ActionType&(fwk.Add|fwk.Update|fwk.Delete)) != 0 {
			hasClaimEvent = true
		}
	}
	if !hasClaimEvent {
		t.Fatalf("EventsToRegister must include ResourceClaim Add|Update|Delete, got %v", events)
	}
}
...
// makeMirroredCacheDevice publishes a capacity that differs from the cache's
// real size, which is what the driver's capacity mirror does to a cache whose
// claims have left it or that another claim squats on.
func makeMirroredCacheDevice(name string, numa int64, partition string, publishedCapacity, trueCacheCPUs int) resourceapi.Device {
	dev := makeCacheDevice(name, numa, partition, publishedCapacity, "")
	largest := int64(trueCacheCPUs)
	dev.Attributes[largestCacheAttr] = resourceapi.DeviceAttribute{IntValue: &largest}
	return dev
}

func TestCacheSizeComesFromTopologyNotMirroredCapacity(t *testing.T) {
	classes := fakeClassLister{"dra.cpu": standardDRADeviceClass()}
	tracker := &fakeClaimTracker{
		claims: map[string]*resourceapi.ResourceClaim{
			"ns/whole": makeTestClaim("whole", "uid-whole", 16, "dra.cpu", "default"),
		},
		// cache-0 is squatted, so the driver floors its published capacity far
		// below the 16 CPUs the cache actually holds.
		allocatedState: &structured.AllocatedState{
			AllocatedDevices: sets.New[structured.DeviceID](),
			AggregatedCapacity: structured.ConsumedCapacityCollection{
				structured.MakeDeviceID(driverName, "cpu-pool", "cache-0"): structured.ConsumedCapacity{
					capacityName: resource.NewQuantity(4, resource.DecimalSI),
				},
			},
		},
	...
	slices := fakeSlices{
		"worker": {
			makeNodeSlice("worker",
				makeMirroredCacheDevice("cache-0", 0, "default", 4, 16),
				makeMirroredCacheDevice("cache-1", 0, "default", 16, 16),
			),
		},
	}
	...
	pod := testPod("p-whole", "whole")
	state := framework.NewCycleState()
	pluginPreFilter(pluginPreFilterArgs{t, plugin, state, pod})

	score, _ := plugin.Score(context.Background(), state, pod, nodeInfoOf(node))

	// Sizing the claim against the first device's published 4 makes k = 4, no
	// NUMA node offers four clean caches, and the claim is scored split. Sized
	// against the topology's 16 it needs one cache, cache-1 is clean, and it is
	// warm-aligned on a node that already holds a claim.
	if score < TierWarmAligned {
		t.Fatalf("whole-cache claim should be warm-aligned once the cache size comes from topology, got %d", score)
	}
}

func TestClaimShapeComesFromTheDriverRequest(t *testing.T) {
	classes := fakeClassLister{
		"dra.cpu":   standardDRADeviceClass(),
		"gpu.class": otherDriverDeviceClass(),
	}

	// The GPU request comes first and carries no dra.cpu capacity. Reading it
	// as the claim's shape sizes the claim at the policy minimum.
	mixed := makeTestClaim("mixed", "uid-mixed", 16, "dra.cpu", "default")
	gpuReq := resourceapi.DeviceRequest{
		Name: "gpu",
		Exactly: &resourceapi.ExactDeviceRequest{
	...
	mixed.Spec.Devices.Requests = append([]resourceapi.DeviceRequest{gpuReq}, mixed.Spec.Devices.Requests...)

	tracker := &fakeClaimTracker{
		claims: map[string]*resourceapi.ResourceClaim{"ns/mixed": mixed},
		allocatedState: &structured.AllocatedState{
			AllocatedDevices: sets.New[structured.DeviceID](),
			AggregatedCapacity: structured.ConsumedCapacityCollection{
				structured.MakeDeviceID(driverName, "cpu-pool", "cache-0"): structured.ConsumedCapacity{
					capacityName: resource.NewQuantity(14, resource.DecimalSI),
				},
			},
		},
	}
	...
	slices := fakeSlices{
		"worker": {
			makeNodeSlice("worker",
				makeCacheDevice("cache-0", 0, "default", 16, ""),
				makeCacheDevice("cache-1", 0, "default", 16, ""),
			),
		},
	}
	...
	pod := testPod("p-mixed", "mixed")
	state := framework.NewCycleState()
	pluginPreFilter(pluginPreFilterArgs{t, plugin, state, pod})

	score, _ := plugin.Score(context.Background(), state, pod, nodeInfoOf(node))

	// Sized from the GPU request the claim is two CPUs, lands on the occupied
	// cache-0 and takes the sub-cache reuse bonus of 8, scoring 88. Sized from
	// the CPU request it needs a whole cache, lands aligned on the clean
	// cache-1, and takes the occupied-cache count of 1, scoring 81.
	if score != TierWarmAligned+1 {
		t.Fatalf("claim should be sized from its dra.cpu request (want %d), got %d", TierWarmAligned+1, score)
	}
}

func TestFilterEvaluatesPodClaimsAgainstOneOverlay(t *testing.T) {
	classes := fakeClassLister{"dra.cpu": standardDRADeviceClass()}
	first := withRepairable(makeTestFlexibleClaim("first", "uid-first", 16, "dra.cpu", "default"))
	second := withRepairable(makeTestFlexibleClaim("second", "uid-second", 16, "dra.cpu", "default"))
	...
	tracker := &fakeClaimTracker{
		...
		// cache-1 already holds 8 CPUs, so the node has exactly one clean cache
		// and 8 spare CPUs beside it.
		allocatedState: &structured.AllocatedState{
			AllocatedDevices: sets.New[structured.DeviceID](),
			AggregatedCapacity: structured.ConsumedCapacityCollection{
				structured.MakeDeviceID(driverName, "cpu-pool", "cache-1"): structured.ConsumedCapacity{
					capacityName: resource.NewQuantity(8, resource.DecimalSI),
				},
			},
		},
	...
	slices := fakeSlices{
		"worker": {
			makeNodeSlice("worker",
				makeCacheDevice("cache-0", 0, "default", 16, "1,1,1,1"),
				makeCacheDevice("cache-1", 0, "default", 16, "1,1,1,1"),
			),
		},
	}

	plugin := setupTestPlugin(tracker, classes, slices, nil)
	pod := testPod("p-two", "first", "second")
	state := framework.NewCycleState()
	pluginPreFilter(pluginPreFilterArgs{t, plugin, state, pod})

	status := plugin.Filter(context.Background(), state, pod, nodeInfoOf(node))

	// The first claim takes the one clean cache. Rebuilding the overlay for the
	// second claim hands it that same cache again and admits a pod the node
	// cannot hold; against one overlay the second claim finds no clean cache
	// and too few CPUs left for its split alternative.
	if status == nil || status.Code() != fwk.Unschedulable {
		t.Fatalf("two whole-cache claims must not both be admitted to a node with one clean cache, got %v", status)
	}
}

func TestFilterRejectsAlignmentWithoutAlternativesEverywhere(t *testing.T) {
	classes := fakeClassLister{"dra.cpu": standardDRADeviceClass()}
	// A sub-cache claim that asks for repair while offering nothing to repair
	// from: a producer error, and not a property of any node.
	claim := withRepairable(makeTestClaim("sub", "uid-sub", 4, "dra.cpu", "default"))
	tracker := &fakeClaimTracker{
		claims: map[string]*resourceapi.ResourceClaim{"ns/sub": claim},
	}

	node := &v1.Node{ObjectMeta: metav1.ObjectMeta{Name: "worker"}}
	// A wholly clean node, where the aligned alternative fits with room to
	// spare: the rejection must not depend on the node being short of caches.
	slices := fakeSlices{
		"worker": {
			makeNodeSlice("worker",
				makeCacheDevice("cache-0", 0, "default", 16, "1,1,1,1"),
	...
	plugin := setupTestPlugin(tracker, classes, slices, nil)
	pod := testPod("p-sub", "sub")
	state := framework.NewCycleState()
	pluginPreFilter(pluginPreFilterArgs{t, plugin, state, pod})

	status := plugin.Filter(context.Background(), state, pod, nodeInfoOf(node))
	if status == nil || status.Code() != fwk.Unschedulable {
		t.Fatalf("alignment without split alternatives must be rejected on every node, got %v", status)
	}
	if !strings.Contains(status.Message(), "no split alternative") {
		t.Fatalf("rejection should name the producer error, got %q", status.Message())
	}
}

func TestFilterRejectsClaimAllocatedInAnotherPool(t *testing.T) {
	classes := fakeClassLister{"dra.cpu": standardDRADeviceClass()}
	// Device names repeat across nodes: both publish "cache-0", and only the
	// pool distinguishes them.
	claim := makeTestClaim("pinned", "uid-pinned", 16, "dra.cpu", "default")
	claim.Status.Allocation = &resourceapi.AllocationResult{
		Devices: resourceapi.DeviceAllocationResult{
			Results: []resourceapi.DeviceRequestAllocationResult{
				{Driver: driverName, Pool: "node-a", Device: "cache-0"},
			},
		},
	}
	tracker := &fakeClaimTracker{
		claims: map[string]*resourceapi.ResourceClaim{"ns/pinned": claim},
	}

	sliceFor := func(node string) *resourceapi.ResourceSlice {
		s := makeNodeSlice(node, makeCacheDevice("cache-0", 0, "default", 16, ""))
		s.Spec.Pool = resourceapi.ResourcePool{Name: node}
		return s
	}
	slices := fakeSlices{
		"node-a": {sliceFor("node-a")},
		"node-b": {sliceFor("node-b")},
	}

	plugin := setupTestPlugin(tracker, classes, slices, nil)
	pod := testPod("p-pinned", "pinned")
	state := framework.NewCycleState()
	pluginPreFilter(pluginPreFilterArgs{t, plugin, state, pod})

	if status := plugin.Filter(context.Background(), state, pod, nodeInfoOf(&v1.Node{ObjectMeta: metav1.ObjectMeta{Name: "node-a"}})); status != nil && !status.IsSuccess() {
		t.Fatalf("Filter on the pool the claim is allocated in should pass, got %v", status)
	}

	status := plugin.Filter(context.Background(), state, pod, nodeInfoOf(&v1.Node{ObjectMeta: metav1.ObjectMeta{Name: "node-b"}}))
	if status == nil || status.Code() != fwk.Unschedulable {
		t.Fatalf("a device name matching in another pool must not read as allocated here, got %v", status)
	}
}

func TestFilterDebitsClaimsItDoesNotGate(t *testing.T) {
	classes := fakeClassLister{"dra.cpu": standardDRADeviceClass()}
	// The first claim passes Filter unconditionally but still takes a cache;
	// the second asks for repair and must be measured against what is left.
	bestEffort := makeTestClaim("best-effort", "uid-be", 16, "dra.cpu", "default")
	repairable := withRepairable(makeTestFlexibleClaim("repairable", "uid-rep", 16, "dra.cpu", "default"))

	tracker := &fakeClaimTracker{
		claims: map[string]*resourceapi.ResourceClaim{
			"ns/best-effort": bestEffort,
			"ns/repairable":  repairable,
		},
		allocatedState: &structured.AllocatedState{
			AllocatedDevices: sets.New[structured.DeviceID](),
			AggregatedCapacity: structured.ConsumedCapacityCollection{
	...
	plugin := setupTestPlugin(tracker, classes, slices, nil)
	pod := testPod("p-mixed", "best-effort", "repairable")
	state := framework.NewCycleState()
	pluginPreFilter(pluginPreFilterArgs{t, plugin, state, pod})

	status := plugin.Filter(context.Background(), state, pod, nodeInfoOf(node))
	if status == nil || status.Code() != fwk.Unschedulable {
		t.Fatalf("a claim Filter does not gate still consumes the cache a later claim needs, got %v", status)
	}
}
